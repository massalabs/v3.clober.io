  ...
  return (
    <React.Fragment key={`${position}-${right}`}>
      <div
        ...
        } w-[2px] bg-gray-400 rounded-sm -top-2.5`}
        style={{
          left: `${position}%`,
        }}
      >
        {label ? (
          <div
            className={`absolute -top-8 w-[${width}px] -right-[22px] flex h-6 px-2 py-1 justify-center items-center gap-1 rounded-2xl bg-blue-500 bg-opacity-10 text-xs text-blue-500 font-bold`}
          >
            {label}
          </div>
        ) : (
